...
interface OrganizationProgressChartProps {
...
const getBarColor = (_progress: number): string => {
  return "#10b981"; // Green color matching Milestone Progress
};

const OrganizationProgressChart: React.FC<OrganizationProgressChartProps> = ({ organizationData, isReport = false }) => {
  ...
  // Sort and limit organizations based on toggle and available space
  const data = useMemo(() => {
    if (!organizationData) return [];
    const sorted = [...organizationData].sort((a, b) =>
  ...
  }, [organizationData, showTop, visibleItems, isReport]);
      ...
      <Stack sx={{ flex: 1, minHeight: 0, justifyContent: isReport ? "flex-start" : "space-evenly", gap: isReport ? 0.5 : 0, pr: 1 }}>
        {data.map((item) => (
          ...
          >
            ...
            <Tooltip title={`${item.organization} - ${item.progress}%`} placement="top">
              <Box sx={{ flex: 1 }}>
                <LinearProgress
                  variant="determinate"
                  value={item.progress}
...
export default memo(OrganizationProgressChart);
