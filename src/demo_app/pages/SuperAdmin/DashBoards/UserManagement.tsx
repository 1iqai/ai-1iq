...
const UserManagement = () => {
  ...
  const handleUpdateUser = async (updatedUser: Partial<User> & { organizationId?: string; divisionIds?: string[] }): Promise<boolean | string> => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/user/update-user/${updatedUser.id}`, {
        ...
        body: JSON.stringify({
          ...
          divisionIds: updatedUser.divisionIds || [],
        }),
      });
  ...
  return (
    <>
      <DashboardHeader
        ...
        breadcrumbs={[{ label: "Dashboard", path: "/super-admin/dashboard" }, { label: "User Management" }]}
      />

      <Box sx={{ padding: {
          xs: 1.5,
          md: 2
      }, borderRadius: 2, backgroundColor: "background.default" }}>
        {/* Toolbar */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
              ...
              >
                <TableHead>
                  <TableRow sx={{ backgroundColor: "background.surface" }}>
                    ...
                    <TableCell style={{ minWidth: 190 }} sx={{ fontWeight: 600 }}>
                      Role
                    </TableCell>
                ...
                <TableBody>
                  {filteredUsers.map((user, index) => (
                    ...
                    >
                      ...
                      {/* Organization Column */}
                      <TableCell>
                        <Typography variant="body2" color={user.organizationName ? "text.primary" : "text.secondary"}>
