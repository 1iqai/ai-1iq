...
const AddOrganizationModal: React.FC<AddOrganizationModalProps> = ({ open, onClose, onSuccess }) => {
  ...
  // const [pendingContractedOrgs, setPendingContractedOrgs] = useState<{ id: string; name: string }[]>([]);
  ...
  // const handleAddPendingContractedOrg = () => {
  //   if (!selectedContractedOrg) return;
  //   setPendingContractedOrgs((prev) => [...prev, selectedContractedOrg]);
  //   setSelectedContractedOrg(null);
  // };

  // const handleRemovePendingContractedOrg = (orgId: string) => {
  //   setPendingContractedOrgs((prev) => prev.filter((o) => o.id !== orgId));
  // };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const result = await fetch(`${import.meta.env.VITE_BACKEND_URL}/organization/create`, {
        method: "POST",
        headers: {
      ...
      if (!result.ok) {
        throw new Error(`Failed to add organization, status: ${result.status}`);
      }

      // const resultData = await result.json();
      // const newOrgId = resultData.data?.id;

      // Add contracted organizations if any were selected
      // if (newOrgId && pendingContractedOrgs.length > 0) {
      //   for (const contractedOrg of pendingContractedOrgs) {
      //     try {
      //       await fetch(`${import.meta.env.VITE_BACKEND_URL}/contracted-organization`, {
      //         method: "POST",
      //         headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      //         body: JSON.stringify({ internalOrgId: newOrgId, contractedOrgId: contractedOrg.id }),
      //       });
      //     } catch {
      //       console.error(`Failed to add contracted org: ${contractedOrg.name}`);
      //     }
      //   }
      // }
              ...
              {/* <Box>
                ...
                >
                  {form.divisionIds.length > 0
                    ? `${form.divisionIds.length} Division(s) Selected`
                    : "Add Teams"}
                </Button>
                {form.divisionIds.length > 0 && (
              ...
              <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                <Autocomplete
                  ...
                  options={allOrganizations.filter(
                    (o) => !pendingContractedOrgs.some((p) => p.id === o.id)
                  )}
                ...
                <Button
                  variant="contained"
                  size="small"
                  onClick={handleAddPendingContractedOrg}
                  disabled={!selectedContractedOrg}
                >
                  Add
                </Button>
              </Box>

              {pendingContractedOrgs.length > 0 ? (
                <List dense disablePadding>
                  {pendingContractedOrgs.map((org) => (
                    <ListItem key={org.id} sx={{ px: 0 }}>
                      <ListItemText primary={org.name} />
                      <ListItemSecondaryAction>
                        <IconButton edge="end" size="small" onClick={() => handleRemovePendingContractedOrg(org.id)}>
                ...
                <Typography variant="body2" color="text.secondary">
                  No contracted organizations added yet.
                </Typography>
              )} */}
            </Box>
          </DialogContent>
